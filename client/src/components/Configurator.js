      ...
      <div className='config__modal__content'>
        <div className='config__modal__model'>
          <Canvas style={{ borderRadius: 8 }}>
            ...
            {item.content === 'object' ?
              <Box scale={3} opacity={config.alpha} color={config.color} hoverDisabled />
              : item.content === 'start' ?
                <Suspense>
                  <Drone position={[0,0,0]}/>
                </Suspense>
                : <group rotation={[-Math.PI/2,0,0]}>
                  <Plane scale={30} color={config.color} transparent onFrame={(state, ref) => {
        ...
        <div>
          {Object.keys(item.config).length > 0 && <>
            <h5>Configuration</h5>
            {config.color && <HuePicker color={config.color} onChangeComplete={(value) => colorOnChange(value)} />}
            {config.alpha && <AlphaPicker color={config.color} onChangeComplete={(value) => colorOnChange(value)} />}
          </>}
        </div>
