...
const createTables = () => {
  db.run(
    'CREATE TABLE IF NOT EXISTS objects (position STRING, id STRING, content STRING, config STRING)',
    (err, rows) => {
...
io.on('connection', (socket) => {
      ...
      db.serialize(() => {
        db.all(query, (err, rows) => {
          const formattedRows = rows
            ...
            .map((el) => ({ ...el, config: JSON.parse(el.config) }))
  ...
  socket.on('set_grid', (payload) => {
    ...
    if (content === 'start' || content === 'end') {
      db.run(
        `UPDATE objects SET content=NULL, config=NULL WHERE content='${content}'`
      )
    }
